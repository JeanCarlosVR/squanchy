...
module.exports = {
  ...
  run: async function run(client, message, args) {
    ...
    let muterole = message.guild.roles.cache.find(role => role.name === "MutedVoice");
    ...
    await tomute
      .send(
        `${muteemoji} You has been temporal muted voice from **${message.guild.name}** for "${reason}" for ${humanizeDuration(timer)}.`
      )
      .catch(m =>
    ...
    await setTimeout(async function() {
      ...
      await tomute
      .send(
        `${muteemoji} You has been unmuted voice from **${message.guild.name}**.`
      )
      .catch(m =>
        message.channel.send({embed: embedNoTell})
      );
      ...
      .setAuthor('Endly Temporal Mute Voice')
      .addField("Moderator", `<@${message.author.id}>`, true)
      .addField("User", `<@${tomute.id}>`, true)
      .setTimestamp(message.createdAt)
      .setColor('#2f3136')
